...
export const execOnDatabase = (cmdenv:IoDatabaseCommandEnvelope,callback:(result:IoDatabaseResponseEnvelope)=>void): void  => {
    ...
    const databaseName = "PersonalTracker"
    
    const tablePomodoros: CrudlTableName = "PomodoroManagement"
    ...
    const handleDatabaseCommand = (
        ...
        DBOpenRequest.onupgradeneeded = (event: IDBVersionChangeEvent) => {
            ...
            if (event.oldVersion < 1){
                ...
                objectStore = database.createObjectStore(tablePomodoros, { keyPath: "id", autoIncrement: true });
